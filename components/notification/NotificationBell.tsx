  ...
  const dropdownContent = (
    <>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        ...
        <div className="flex items-center gap-2">
          {unreadCount > 0 && (
            <button onClick={() => markAllRead()} className="text-[11px] text-[#C9A227] hover:underline font-medium">
              Mark all read
            </button>
          )}
          <button onClick={() => { setOpen(false); router.push('/notifications'); }} className="text-[11px] text-gray-500 hover:text-[#1F2937]">
            View all →
          </button>
        </div>
      </div>
      ...
      {notifications.length > 5 && (
        <div className="px-4 py-2.5 border-t border-gray-100 text-center">
          <button onClick={() => { setOpen(false); router.push('/notifications'); }} className="text-xs text-[#C9A227] hover:underline font-medium">
  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      {open && placement !== 'sidebar' && (
        <div className="absolute right-0 top-11 w-72 sm:w-80 bg-white rounded-2xl border border-gray-100 shadow-xl z-50 overflow-hidden">
          {dropdownContent}
        </div>
      )}
    </div>
