...
export function ChatWindow({ conversationId, currentUserId }: ChatWindowProps) {
  ...
  const removeImage = useCallback((idx: number) => {
    ...
    if (newImgs.length === 0) { setPendingKind(null); setMediaLabel(''); }
  ...
  const removeDoc = useCallback((idx: number) => {
    ...
    if (newDocs.length === 0) { setPendingKind(null); setMediaLabel(''); }
  }, [pendingDocs]);
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (blobs) { blobs.forEach(URL.revokeObjectURL); blobUrlsMap.current.delete(optimisticId); }
  ...
  if (isLoading) {
    return (
      <div className="flex-1 flex flex-col h-full">
        <div className="flex items-center gap-3 px-3 sm:px-4 py-3 border-b border-gray-100 bg-white animate-pulse shrink-0">
        ...
        <div className="flex-1 bg-[#F8F9FB] p-3 sm:p-4 space-y-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className={`flex ${i % 2 ? 'justify-start' : 'justify-end'}`}>
              <div className="h-10 bg-gray-200 rounded-2xl animate-pulse" style={{ width: `${120 + i * 30}px` }} />
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col h-full min-w-0">
      {/* ── Header ──────────────────────────────────────────────────── */}
      <div className="flex items-center gap-2 sm:gap-3 px-2 sm:px-4 py-2.5 border-b border-gray-100 bg-white shrink-0">
        <button
        ...
        <div className="relative shrink-0">
          {participant.avatar ? (
            ...
            <img src={participant.avatar} alt={participant.name} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full object-cover" />
          ) : (
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-linear-to-br from-[#C9A227] to-[#D4AF37] flex items-center justify-center text-white font-bold text-sm">
              {initials}
            </div>
          )}
          {participant.is_online && (
            <span className="absolute bottom-0 right-0 w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-green-400 border-2 border-white" />
          )}
        </div>
        ...
        <span className="hidden sm:inline text-[10px] text-gray-400 bg-gray-100 px-2 py-1 rounded-full font-mono shrink-0">
        ...
        {grouped.map(({ date, messages: dayMsgs }) => (
          <div key={date}>
            <div className="flex items-center gap-2 sm:gap-3 my-3 sm:my-4">
              ...
              <span className="text-[10px] sm:text-[11px] text-gray-400 font-medium px-2 whitespace-nowrap">{date}</span>
              <div className="flex-1 h-px bg-gray-200" />
            </div>
        ...
        <div className="shrink-0 bg-white border-t border-gray-100 px-2 sm:px-3 py-2 max-h-48 overflow-y-auto">
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center gap-1 flex-wrap">
                {imagePreviews.map((url, i) => (
                  <div key={i} className="relative w-14 h-14 sm:w-16 sm:h-16 rounded-xl overflow-hidden border border-gray-200 shrink-0">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          {pendingKind === 'video' && pendingVideo && (
            <div className="flex items-center gap-2">
              {videoPreview && <video src={videoPreview} className="w-14 h-14 object-cover rounded-xl border border-gray-200 shrink-0" muted />}
              <div className="flex-1 min-w-0">
              ...
              <button onClick={clearPending} className="w-6 h-6 rounded-full bg-gray-100 hover:bg-red-100 text-gray-500 hover:text-red-500 flex items-center justify-center text-xs transition-colors shrink-0">✕</button>
            </div>
          )}
          ...
          {pendingKind === 'docs' && (
            <div className="flex flex-col gap-1">
              {pendingDocs.map((f, i) => (
                <div key={i} className="flex items-center gap-2 bg-gray-50 rounded-xl px-2 sm:px-3 py-1.5">
                  <div className="w-7 h-7 rounded-lg bg-[#C9A227]/10 flex items-center justify-center text-[#C9A227] shrink-0">
                    <span className="text-[10px] font-bold">{(f.name.split('.').pop() ?? 'DOC').toUpperCase().slice(0,3)}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <button onClick={() => removeDoc(i)} className="w-5 h-5 rounded-full hover:bg-red-100 text-gray-400 hover:text-red-500 flex items-center justify-center text-[10px] transition-colors">✕</button>
                </div>
              ))}
              {pendingDocs.length < MAX_FILES && (
                <button onClick={() => fileRef.current?.click()} className="text-xs text-[#C9A227] hover:underline text-left pl-1">+ Add file (max {MAX_FILES})</button>
              )}
            </div>
          )}
          ...
          <div className="mt-1.5 flex items-center gap-1.5">
            ...
            />
            <button onClick={clearPending} className="text-xs text-gray-400 hover:text-red-500 transition-colors whitespace-nowrap px-1 shrink-0">Cancel</button>
          </div>
        </div>
      )}
      ...
      <div className="shrink-0 bg-white border-t border-gray-100 px-2 sm:px-3 py-2 sm:py-3">
        ...
        <input ref={imageRef} type="file" accept={ACCEPT_IMAGE} multiple className="hidden"
          onChange={(e) => { const files = Array.from(e.target.files ?? []); if (pendingKind === null || pendingKind === 'images') handleImagesPick(files); e.target.value = ''; }} />
        <input ref={videoRef} type="file" accept={ACCEPT_VIDEO} className="hidden"
          onChange={(e) => { const f = e.target.files?.[0]; if (f) handleVideoPick(f); e.target.value = ''; }} />
        <input ref={fileRef} type="file" accept={ACCEPT_FILE} multiple className="hidden"
          onChange={(e) => { const files = Array.from(e.target.files ?? []); if (pendingKind === null || pendingKind === 'docs') handleDocsPick(files); e.target.value = ''; }} />

        <div className="flex items-end gap-1 sm:gap-2">
          {/* Attachment buttons */}
          <div className="flex items-center gap-0.5 sm:gap-1 mb-0.5 shrink-0">
            <button
              onClick={() => { if (!pendingKind || pendingKind === 'images') imageRef.current?.click(); }}
            ...
            <button
              onClick={() => { if (!pendingKind || pendingKind === 'video') videoRef.current?.click(); }}
            ...
            <button
              onClick={() => { if (!pendingKind || pendingKind === 'docs') fileRef.current?.click(); }}
