    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-4 flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div>
            <Link href={profileUrl} className="font-semibold text-[#1F2937] hover:text-[#C9A227] transition-colors">
              {profile.name}
            </Link>
        ...
        {(['received', 'sent'] as Tab[]).map((t) => (
          ...
          >
            ...
            {t === 'received' && total > 0 && tab === 'received' && (
              <span className="ml-1.5 bg-[#C9A227] text-white text-xs rounded-full px-1.5 py-0.5">{total}</span>
            )}
          </button>
        ))}
      </div>
